...
impl MarchingTileBundle {
    ...
    pub fn cell_mesh(cell: &TileCell) -> Mesh {
        let size = *LazyLock::force(&CHUNK_SIZE_XYZ);
        let mut mesh = match cell.id.type_value() {
            TileID::FLOOR => Self::floor_mesh(size),
            TileID::WALL => Self::wall_mesh(size),
            TileID::CORNER => Self::corner_mesh(size),
            _ => panic!("no mesh for {}", cell.id.to_string()),
        };

        let local = cell.local_transform(size);
        mesh.transform_by(local);
    ...
    pub fn global_transform(cell: &TileCell, origin: &Transform) -> GlobalTransform {
        ...
        // we swap the x and z coordinates and subtract them by 1. This ensures the cell at
        // (1,1) is placed at the origin
